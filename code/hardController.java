...
public class hardController implements Initializable{
		...
		move = new Timeline(new KeyFrame(Duration.millis(m),(e)-> {
			...
			if(x>man.getLayoutX()&& x<man.getLayoutX()+man.getFitWidth()) {
				if(y>man.getLayoutY()+man.getFitHeight()/2) {
					Z.setLayoutY(Z.getLayoutY()-15);
					Z.setRotate(180);
					if(x>man.getLayoutX()+15 && x<man.getLayoutX()+40) {
						Z.setLayoutX(Z.getLayoutX()+15);
					}
	...
	public void best() {
		if(min<bestmin) {
			...
			MenuCotroller.threemin=min;
			MenuCotroller.threesec=sec;
			rec.setVisible(true);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		if(MenuCotroller.threemin!=1000) {
			bestmin=MenuCotroller.threemin;
			bestsec=MenuCotroller.threesec;
			BS.setText("BestScore:"+String.format("%02d",MenuCotroller.threemin)+":"+String.format("%02d", MenuCotroller.threesec));
		}
		blood=100;
		record=0;
		...
		manmove = new Timeline(new KeyFrame(Duration.millis(50),(e)-> {
			if(up) {
				if(man.getLayoutY()<0) man.setLayoutY(man.getLayoutY());
					...
					man.setLayoutY(man.getLayoutY()-10-plus);
					man.setRotate(0);
				}
			}
			if(left) {
				if(man.getLayoutX()<0) man.setLayoutX(man.getLayoutX());
					...
					man.setLayoutX(man.getLayoutX()-10-plus);
					man.setRotate(270);
				}
			}
			if(down) {
		...
		MakeZ=new Timeline(new KeyFrame(Duration.millis(500),(e)->{
			int i = (int) (Math.random() * (960));
			int n = (int) (Math.random() * (780));
			int m=(int)(Math.random()*4);
			Image img = new Image("file:C:\\Users\\dell\\Desktop\\javaTest\\javatest\\src\\project\\hardZombie.png");
			...
			if(m==0) {
				...
				newZombie.setLayoutX(i);
				newZombie.setLayoutY(774);
				newZombie.setRotate(180);
			}
			else if(m==2) {
				newZombie.setLayoutX(0);
				newZombie.setLayoutY(n);
		...
		rc1=new Timeline(new KeyFrame(Duration.millis(200),(e)->{
			record=0;
		}));
